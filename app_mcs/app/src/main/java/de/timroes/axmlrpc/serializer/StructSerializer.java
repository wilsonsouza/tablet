...
public class StructSerializer implements Serializer
{
   ...
   public Object deserialize(Element content) throws XMLRPCException
   {
      ...
      for (int i = 0; i < content.getChildNodes().getLength(); i++)
      {
         ...
         if ((n.getNodeType() == Node.TEXT_NODE && n.getNodeValue().trim().length() <= 0) || n.getNodeType() == Node.COMMENT_NODE)
         {
            continue;
         }

         if (n.getNodeType() != Node.ELEMENT_NODE || !STRUCT_MEMBER.equals(n.getNodeName()))
         {
         ...
         for (int j = 0; j < n.getChildNodes().getLength(); j++)
         {
            ...
            // Strip only whitespace text elements and comments
            if ((m.getNodeType() == Node.TEXT_NODE && m.getNodeValue().trim().length() <= 0) || m.getNodeType() == Node.COMMENT_NODE)
            {
               continue;
            }
